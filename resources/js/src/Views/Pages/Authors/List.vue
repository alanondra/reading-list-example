		...
		<load-spinner :loaded="loaded">
			<b-container>
				<b-form @submit="update">
					<b-row>
						<b-col md="12" lg="8">
							<h2>{{ trans('app.navbar.authors') }}</h2>
						</b-col>
					...
					<b-row class="my-2">
						<b-col cols="12" v-if="authors.length === 0">
							{{ trans('authors.empty') }}
						</b-col>
						<b-col cols="12" v-else>
							<b-table striped :items="authors" :fields="fields">
...
<script>
	export default {
		...
		methods: {
			...
			update() {
				return axios.get(this.apiRoute('api.authors.index', this.input))
					.then((response) => {
